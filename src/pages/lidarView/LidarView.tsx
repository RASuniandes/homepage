...
export default function LidarView () {
  ...
  function addPoint (x: number, y: number) {
    ...
    point.className =
      'absolute w-1.5 h-1.5 bg-yellow-400 rounded-full shadow-[0_0_8px_#FAAE1F] z-20 transition-opacity duration-1000'
    point.style.left = `${x}%`
    point.style.top = `${y}%`
  ...
  useEffect(() => {
    const ros = new ROSLIB.Ros({ url: 'wss://192.168.0.108:9090' })
  ...
  useEffect(() => {
    ...
    scanTopic.subscribe((msg: unknown) => {
      const { ranges, angle_min, angle_increment } = msg as {
        ranges: number[]
        angle_min: number
  ...
  return (
    <div
      className='min-h-screen bg-black text-white p-6 overflow-hidden select-none'
    ...
    >
      ...
      <div className='max-w-7xl mx-auto flex justify-between items-start mb-8 mt-12'>
        ...
        >
          <h1
            className='text-4xl font-black italic tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-red-600 to-yellow-500'
          ...
          <div className='flex items-center gap-2 mt-1'>
            <Radio
              className={`w-4 h-4 ${
                status === 'CONNECTED' ? 'text-green-500' : 'text-red-600'
              }`}
            />
            <span className='text-xs font-mono tracking-widest uppercase'>
              {status} // 192.168.0.108
            </span>
          </div>
        </motion.div>
      ...
      <div className='relative flex justify-center items-center h-[70vh]'>
        ...
        >
          ...
          {/* Compass Markers */}
          <span className='absolute top-4 left-1/2 -translate-x-1/2 text-xs font-bold text-red-700'>
            000°
          </span>
          <span className='absolute bottom-4 left-1/2 -translate-x-1/2 text-xs font-bold text-red-700'>
            180°
          </span>
          <span className='absolute right-4 top-1/2 -translate-y-1/2 text-xs font-bold text-red-700 font-mono'>
            090°
          </span>
          <span className='absolute left-4 top-1/2 -translate-y-1/2 text-xs font-bold text-red-700 font-mono'>
            270°
          </span>
